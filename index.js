...
bot.onText(/\/start/, function (msg) {
    bot.sendMessage(msg.from.id, 'Розклад - розклад пар \n' +
        ...
        'Інформація про факультет - інформація про факультет\n' +
        'Список предметів - your_moodle_login - your_moodle_password - список предметів доступних для Вас у Moodle\n' +
		'Додаткова інформація - інформація про розташування корпусів та додаткових приміщень');
});
...
bot.on('message', function (msg) {
    ...
    if (messageText === 'hello') {
    ...
    } else if (messageText === 'інформація про факультет') {
		...
		bot.sendMessage(id, mess);
    } else if (messageText === 'додаткова інформація') {
        bot.sendMessage(id, "Оберіть пункт", {
            "reply_markup": {
